...
func TestRequestHttp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Single GET request success",
			attr: RequestAttributes{
				Method: GET,
				URL:    "",
			},
		...
		{
			name: "Single POST request with body",
			attr: RequestAttributes{
				Method: POST,
				URL:    "",
				Body:   TMap{"key": "value"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := RequestHttp(tt.attr, detailLog, summaryLog)
			if (err != nil) != tt.expectError {
				t.Errorf("RequestHttp() error = %v, expectError %v", err, tt.expectError)
				return
			}

			if !equalResponses(result, tt.expectedBody) {
