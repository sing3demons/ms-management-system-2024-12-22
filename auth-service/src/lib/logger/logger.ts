...
function loadLogConfig(
  ...
  if (config?.LogConfig) {
    if (config.LogConfig.detail) {
      confLog.detail = config.LogConfig.detail
    }
    if (config.LogConfig.summary) {
      confLog.summary = config.LogConfig.summary
    }
  }
  ...
  if (process.env.PORT) {
    config.Addr = process.env.PORT
  }

  process.env.PORT = config.Addr || '3000'
...
export { getFileName, createStreams, confLog, LogConfig, loadLogConfig, writeLogFile }
