...
class BaseRoute {
  ...
  protected addRoute<T extends string, P extends TP<T>, B extends TS, Q extends TS>(
    ...
    if (path) {
      path = path.startsWith('/') ? path : (`/${path}` as T)
    }
...
interface IServer {
  ...
  routes: (...routers: AppRouter[]) => IServer
}
...
class AppServer implements IServer {
  ...
  constructor(before?: Awaited<ReturnType<typeof Function>>) {
    if (before) {
      before()
    }
  ...
  public routes(...routers: AppRouter[]) {
  ...
  public listen(port: number | string, close?: Awaited<ReturnType<typeof Function>>) {
    ...
    const server = http.createServer(this.app).listen(port, () => console.log(`Server running on port ${port}`))
